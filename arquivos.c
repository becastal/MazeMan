...
void salva_labirinto_mazeman(labirinto* L) {
    char nome_arquivo[100];
    char nome_completo[120];
    FILE* arquivo;
    int ok_nome = 0;
    ...
    while (!ok_nome) {
        ...
        int tamanho = snprintf(nome_completo, sizeof(nome_completo), "./mazeman/%s.txt", nome_arquivo);
    ...
    fclose(arquivo);
    printf("[i] labirinto do MazeMan salvo em: %s\n", nome_completo);
}
